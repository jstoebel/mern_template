...
export function getLogin({user}, res) {
  if (user) {
    return res.redirect('/');
  }
...
export function postLogin(req, res, next) {
  ...
  req.sanitize('email').normalizeEmail({remove_dots: false});
    ...
    // eslint-disable-next-line require-jsdoc
    function setUserInfo({_id, profile, email, role}) {
        return {
            _id: _id,
...
export function getSignup({user}, res) {
  if (user) {
    return res.redirect('/');
  }
...
export function getOauthUnlink(req, res, next) {
  ...
  User.findById(req.user.id, (err, user) => {
    ...
    user.tokens = _.reject(user.tokens, ({kind}) => kind === provider);
    user.save((err) => {
      if (err) {
        return next(err);
      }
